...
Transformation to uncorrelated Gaussian variables: Eigenvalues and eigenvectores of
...
# Example 6-10 - Ang & Tang, 1984, pag. 218
namevar=np.array(['fy','Z','Mu'])
...
# Correlation matrix
#
Rz=np.array([[1.00,0.40,0.00],[0.40,1.00,0.00],[0.00,0.00,1.00]])
#
...
Imatrix=np.eye(n)
#
# Eigenvalue and eigenvectors of the correlation matrix
#
eigvals, eigvecs =scipy.linalg.eig(Rz)
Lambdamatrix=eigvals.real*Imatrix
Amatrix_ortogonal=np.copy(eigvecs.real)
Amatrix=np.dot(Amatrix_ortogonal,np.linalg.inv(Lambdamatrix**(1./2.)))
Jyz=np.transpose(Amatrix)
Jzy=np.linalg.inv(np.transpose(Amatrix))
#
